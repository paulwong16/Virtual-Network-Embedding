function [NetNode,dataL]=SalamNet_NetCreate(BorderLength,NodeAmount, ...,
...
[NetNode,dataL]=Net_plot(BorderLength,NodeAmount,Sxy,EdgeCost,EdgeBandWide,VertexCost,PlotIf);
end

%用于绘制网络拓扑的函数
function [NetNode,dataL]=Net_plot(BorderLength,NodeAmount,Sxy,EdgeCost,EdgeBandWide,VertexCost,PlotIf)
%画节点
figure(1);
if PlotIf == 1
    ...
    for i=1:NodeAmount
        ...
        for j=1:NodeAmount
            if i ~= j
                if isinf(EdgeCost(i,j)) == 0
                    EC=EC+EdgeCost(i,j);
                    EBW=EBW+EdgeBandWide(i,j);
                end
            end
        end
        CC=1/EC;
        DC=EBW;
        NetNode=[NetNode;i,NodeX,NodeY,NR,CC,DC,1];
    end
end
xlswrite('link.xlsx',dataL);
xlswrite('node.xlsx',dataN);
if PlotIf == 1
